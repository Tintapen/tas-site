<?php

namespace App\Filament\Resources;

use App\Filament\Resources\ReferenceResource\Pages;
use App\Filament\Resources\ReferenceResource\RelationManagers;
use App\Models\Reference;
use Filament\Forms;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables;
use Filament\Tables\Table;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\SoftDeletingScope;
use Filament\Forms\Components\TextInput;
use Filament\Forms\Components\Toggle;
use Filament\Forms\Components\Repeater;
use Filament\Forms\Components\Textarea;
use Filament\Forms\Components\Section;
use Filament\Forms\Components\Placeholder;
use Filament\Tables\Columns\TextColumn;
use Filament\Tables\Columns\BadgeColumn;
use Filament\Tables\Filters\SelectFilter;

class ReferenceResource extends BaseResource
{
    protected static ?string $model = Reference::class;

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Reference')
                    ->schema([
                        TextInput::make('name')
                            ->label('Name')
                            ->required(),
                        Toggle::make('isactive')
                            ->label('Active')
                            ->inline(false)
                            ->default(true)
                            ->formatStateUsing(fn ($state) => $state === 'Y' || $state === true || is_null($state))
                            ->dehydrateStateUsing(fn ($state) => $state ? 'Y' : 'N'),
                        Textarea::make('description')
                            ->label('Description')
                            ->rows(4),
                    ])
                    ->columns(2),

                Section::make('Detail')
                    ->schema(fn (callable $get) => $get('id') ? [
                        Repeater::make('referenceDetails')
                            ->label(false)
                            ->relationship()
                            ->schema([
                                TextInput::make('value')
                                    ->label('Value')
                                    ->required()
                                    ->columnSpan(1),
                                TextInput::make('name')
                                    ->label('Name')
                                    ->required()
                                    ->columnSpan(1),
                                Toggle::make('isactive')
                                    ->label('Active')
                                    ->inline(false)
                                    ->default(true)
                                    ->formatStateUsing(fn ($state) => $state === 'Y' || $state === true || is_null($state))
                                    ->dehydrateStateUsing(fn ($state) => $state ? 'Y' : 'N')
                                    ->required()
                                    ->columnSpan(1),
                            ])
                            ->columns(3) // 2 kolom per baris
                            ->defaultItems(2), // jika ingin tampil 2 baris default
                    ] : [
                        Placeholder::make('save-first')
                            ->content('🛑 Please save the Reference Header first before adding details.'),
                    ]),
            ]);
    }

    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                TextColumn::make('name')
                    ->label('Name')
                    ->searchable(),
                TextColumn::make('description')
                    ->label('Descritpion')
                    ->searchable(),
                TextColumn::make('reference_details_count')
                    ->label('Total Details'),
                BadgeColumn::make('isactive')
                    ->label('Status')
                    ->formatStateUsing(fn (string $state): string => $state === 'Y' ? 'Active' : 'Nonactive')
                    ->color(fn (string $state): string => $state === 'Y' ? 'success' : 'danger'),
            ])
            ->defaultSort('name')
            ->filters([
                SelectFilter::make('isactive')
                    ->label('Status')
                    ->options([
                        'Y' => 'Active',
                        'N' => 'Nonactive',
                    ]),
            ])
            ->actions([
                Tables\Actions\EditAction::make(),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make(),
                ]),
            ]);
    }

    public static function getRelations(): array
    {
        return [
            //
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListReferences::route('/'),
            'create' => Pages\CreateReference::route('/create'),
            'edit' => Pages\EditReference::route('/{record}/edit'),
        ];
    }

    public static function getEloquentQuery(): Builder
    {
        return parent::getEloquentQuery()->withCount('referenceDetails');
    }
}
